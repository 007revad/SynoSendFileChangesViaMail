...
# --------- collection of parameters for send to admin
...
# --------- creation REP_DATA for files CURR_FILE & PRIOR_FILE
...
# --------- writing the contents of the folder passed in CURR_FILE
...
# --------- compare PRIOR_FILE to CURR_FILE

if [ -f "$REP_DATA/$PRIOR_FILE" ]; then
    # --------- the 'prior' file exists: comparison possible
...
# ----------- current list copy (CURR_FILE) for future comparison

cp "$REP_DATA/$CURR_FILE" "$REP_DATA/$PRIOR_FILE"


# ----------- send email to admin or users

if [ "$RESULT_ALL" != "" ]; then
   #/usr/bin/php -r "mail('$USERS_MAIL_LIST', '$EMAIL_TITLE', '$EMAIL_INTRO $RESULT_ALL', 'From: $FROM_MAIL');";
   #/usr/bin/php -r "mail('$USERS_MAIL_LIST', '$EMAIL_TITLE', '$RESULT_ALL', 'From: $FROM_MAIL');";
   #/usr/bin/php -r "mail('$USERS_MAIL_LIST', '$EMAIL_TITLE', '$EMAIL_INTRO'."\n".'$RESULT_ALL', 'From: $FROM_MAIL');";
   #/usr/bin/php -r "mail('$USERS_MAIL_LIST', '$EMAIL_TITLE', '$EMAIL_INTRO'."<br>".'$RESULT_ALL', 'From: $FROM_MAIL');";
   #/usr/bin/php -r "mail('$USERS_MAIL_LIST', '$EMAIL_TITLE', '$EMAIL_INTRO'."\r\n".'$RESULT_ALL', 'From: $FROM_MAIL');";
   /usr/bin/php -r "mail('$USERS_MAIL_LIST', '$EMAIL_TITLE', '$RESULT_ALL', 'From: $FROM_MAIL');";
else
